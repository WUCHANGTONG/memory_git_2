...
class HealthSafety(BaseModel):
    ...
    daily_energy_level: FieldValue     # 精力水平 → 任务复杂度控制
    risk_sensitivity_level: FieldValue # 风险敏感度 → 建议谨慎程度
```
...
class CognitiveInteraction(BaseModel):
    ...
    processing_speed: FieldValue              # 信息处理速度 → 呈现节奏控制
    digital_literacy: FieldValue              # 数字技能 → 技术术语使用
    instruction_following_ability: FieldValue # 指令理解能力 → 步骤拆分程度
```
...
class EmotionalSupport(BaseModel):
    ...
    emotional_support_need: FieldValue     # 情感支持需求 → 是否先安慰
...
class ValuesPreferences(BaseModel):
    ...
    value_orientation: FieldValue     # 价值观导向 → 建议方向对齐
    motivational_factors: FieldValue  # 激励因素 → 增强情感共鸣
```
...
def get_generation_control_params(self) -> Dict[str, Any]:
    ...
    return {
        ...
        # 情感支持
        "emotional_support_need": "moderate", # low/moderate/high
        "loneliness_level": "low",           # low/moderate/high
        
        # 个性化内容
        "core_interests": ["园艺", "太极"],
        "taboo_topics": ["政治", "金钱"],
        "motivational_factors": ["家庭", "健康"]
    }
```
...
2. **认知适配控制**：注意力持续时间、信息处理速度
3. **情感支持控制**：情感支持需求强度、孤独感程度
4. **内容个性化控制**：核心兴趣、敏感话题、激励因素
