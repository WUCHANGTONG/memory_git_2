...
#（画像更新器）
# 加载 .env 文件（从项目根目录查找）
env_path = Path(__file__).parent.parent / '.env'  # 项目根目录的 .env 文件
if env_path.exists():
...
def init_llm() -> ChatTongyi:
    """
    初始化LLM（延迟初始化），如果API Key无效会抛出异常
    
    Returns:
        ChatTongyi: 初始化后的通义千问LLM实例
    ...
    if llm is None:
        ...
        try:
            llm = ChatTongyi(
                ...
                model_name="qwen-turbo",  # 使用通义千问turbo模型
                temperature=0  # 温度设为0保证输出稳定
            )
...
def update_profile(conversation: str, profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 格式化prompt
        messages = profile_prompt.format_messages(
        ...
        print("\n请检查以下几点：")
        print("1. 确认 .env 文件位于项目根目录（memory目录）")
        ...
        print("\n如果问题仍然存在，请检查：")
        print("- 登录阿里云DashScope控制台 (https://dashscope.console.aliyun.com/)")
        print("- 确认API Key是否正确复制（注意前后是否有空格）")
        print("- 确认API Key是否有调用通义千问模型的权限")
        print("- 确认账户余额是否充足")
        return profile
    except Exception as e:
        error_str = str(e)
        ...
        if "401" in error_str or "InvalidApiKey" in error_str or "Invalid API-key" in error_str:
            ...
            if key_info['status'] == 'configured':
                ...
                print("如果前8位不正确，说明API Key可能配置错误")
        else:
            print(f"❌ 画像更新失败: {error_str}")
